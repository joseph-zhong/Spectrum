...
def name2cp(k):
  ...
  if hasattr(htmlentitydefs, "name2codepoint"):  # requires Python 2.3
    ...
    if k.startswith("&#") and k.endswith(";"): return int(
      k[2:-1])  # not in latin-1
...
def dumb_property_dict(style):
  ...
  return dict([(x.strip(), y.strip()) for x, y in
               [z.split(':', 1) for z in style.split(';') if ':' in z]]);
      ...
      if self.a and (
        ...
        for link in self.a:
          if self.outcount > link['outcount']:
            self.out("   [" + str(link['count']) + "]: " + urlparse.urljoin(
              self.baseurl, link['href']))
...
if __name__ == "__main__":
  ...
  p.add_option("-i", "--google-list-indent", dest="list_indent", action="store",
               type="int",
